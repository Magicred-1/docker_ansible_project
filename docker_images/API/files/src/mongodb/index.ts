...
class MongoDBConnector {
  ...
  async vehiculeDelete(vehiculeID: Vehicule['_id']) {
    try {
      ...
      const searchedVehicule = await vehicule.findOne({
        _id: new mongodb.ObjectId(vehiculeID),
      });
      ...
      const result = await vehicule.deleteOne({
        _id: new mongodb.ObjectId(vehiculeID),
      });
  ...
  async vehiculeUpdate(
    ...
    try {
      ...
      const searchedVehicule = await vehicules.findOne({
        _id: new mongodb.ObjectId(vehiculeID),
      });
      if (searchedVehicule == undefined) {
  ...
  async carsitterAdd(carsitter: Carsitter) {
    try {
      ...
      const searchedCarsitter = await carsitters.findOne({
        _id: new mongodb.ObjectId(carsitter['_id']),
      });
  ...
  async planningAdd(planning: Planning) {
    try {
      ...
      const searchedCarsitter = await carsitters.findOne({
        _id: new mongodb.ObjectId(planning.carsitterID),
      });
      const searchedVehicule = await vehicules.findOne({
        _id: new mongodb.ObjectId(planning.vehiculeID),
      });
      const searchedClient = await clients.findOne({
        _id: new mongodb.ObjectId(planning.clientID),
      });
      const searchedPlanning = await plannings.findOne({
        _id: new mongodb.ObjectId(planning['_id']),
      });

      if (
        searchedCarsitter?._id === searchedVehicule?._id ||
        searchedCarsitter?._id === searchedClient?._id ||
        searchedVehicule?._id === searchedClient?._id
      ) {
      ...
      if (
        insertedPlanning.date == undefined ||
        insertedPlanning.time == undefined ||
        insertedPlanning.duration == undefined
      ) {
        throw new Error('Date, time or duration is undefined');
      }

      if (
        String(insertedPlanning.date).match(/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/) ==
          null ||
        String(insertedPlanning.time).match(/^[0-9]{1,2}$/) == null ||
        String(insertedPlanning.duration).match(/^[0-9]{1,2}$/) == null
      ) {
  ...
  async planningDelete(planningID: Planning['_id']) {
    try {
      ...
      const result = await plannings.deleteOne({
        _id: new mongodb.ObjectId(planningID),
      });
  ...
  async planningGetById(planningID: Planning['_id']) {
    try {
      ...
      const result = await plannings.findOne({
        _id: new mongodb.ObjectId(planningID),
      });
